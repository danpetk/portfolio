...
function GlowingUnderlinedText({children, anchor, action, yellow=false}) {
    ...
    if(anchor){
        if(yellow){
            return ( 
                <>  
                    <a className={`relative inline-block text-yellow-400 drop-shadow-glowYellow group`} href={action}>{children}
        ...
        if(yellow){
            return ( 
                <>  
                    <span className={`relative inline-block cursor-pointer text-yellow-400 drop-shadow-glowYellow group`} onClick={action}>{children}
                        <span className={`absolute left-0 bottom-0 w-full h-0.5 bg-yellow-400 scale-x-0 origin-left transition-transform duration-300 ease-in-out group-hover:scale-x-100 rounded-lg`}></span>
                    </span> 
                </>
            )
